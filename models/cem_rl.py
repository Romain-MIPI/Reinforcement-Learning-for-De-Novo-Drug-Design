...
class CEM_RL(object):
    def __init__(self, generator, predictor, get_reward):
        ...
        super(CEM_RL, self).__init__()
        self.generator = generator
        self.predictor = predictor
        self.get_reward = get_reward

    def cross_entropy(self, path, data, n_batch=10, gamma=0.97,
        ...
        # updating dataset with elites
        elites_samples = np.array(samples)[np.where(rewards > 1)]
        elites_labels = np.array([self.predictor.predict(sample) for sample in samples])

        data.update_elite(path, elites_samples, elites_labels)
